...
$error = '';
$success = '';
$loginInfo = ''; // для показа логина/пароля при регистрации
...
// ========= Работа с Cookies для незалогиненных =========
...
// ========= Обработка логина (если нажал "Войти") =========
$loginError = '';
...
// ========= Заполняем форму данными (если авторизован — из БД, иначе из Cookies) =========
$full_name = '';
$phone = '';
$email = '';
$birth_date = '';
...
if (isset($_SESSION['user_id'])) {
    ...
    // Неавторизован — берём из Cookies
    ...
    if (empty($errors)) {
        try {
            ...
            if (isset($_SESSION['user_id'])) {
                ...
                $login = 'user_' . bin2hex(random_bytes(4)); // например, user_a1b2c3d4
                $plainPassword = bin2hex(random_bytes(3));    // например, a1b2c3
        ...
        // Сохраняем в Cookies только для незалогиненных
        if (!isset($_SESSION['user_id'])) {
...
<body>
    <div class="container">
        <h1>📝 Регистрационная форма (lab5)</h1>

        <?php if ($loginInfo): ?>
            <div class="success">
                🎉 <?= $loginInfo ?>
                <br><small>Сохраните эти данные для входа!</small>
            </div>
        <?php endif; ?>
