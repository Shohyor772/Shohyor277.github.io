...
// Редактирование пользователя (с проверкой CSRF)
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (empty($errors)) {
            try {
                ...
                $pdo->rollBack();
                error_log($e->getMessage());
                $message = "Ошибка БД при обновлении.";
            }
        } else {
...
// Список всех пользователей (с экранированием)
$usersStmt = $pdo->query("
    SELECT u.*, 
...
<body>
    <div class="container">
        <h1>🛡️ Админ-панель (защищённая)</h1>

        <?php if ($message): ?>
            <div class="message">ℹ️ <?= htmlspecialchars($message) ?></div>
        <?php endif; ?>
        ...
        <?php if ($editUser): ?>
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="birth_date" value="<?= htmlspecialchars($editUser['birth_date']) ?>" required>
                </div>
                <div class="form-group">
                    <label>Пол</label>
                    <select name="gender">
                ...
                <div class="form-group">
                    ...
                    <select name="languages[]" multiple>
                        ...
                        foreach ($allLangs as $lang): ?>
                            <option value="<?= htmlspecialchars($lang) ?>" <?= in_array($lang, $editLanguages) ? 'selected' : '' ?>><?= htmlspecialchars($lang) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
            ...
            <div class="stats">
                <?php foreach ($stats as $stat): ?>
                    <div class="stat-card">
                        ...
                        <?= htmlspecialchars($stat['cnt']) ?> пользователь(ей)
                    </div>
                <?php endforeach; ?>
            </div>
